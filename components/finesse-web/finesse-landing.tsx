 ...
 * the brand; SevenF appears ONLY in the footer credit. Every CTA preserves the
 * explicit product context so the login renders as Finesse from first paint.
 */

export const FINESSE_LOGIN_HREF = "/login?product=finesse"
    ...
    >
      ...
      <main id="contenido">
        <section className={`${SECTION} grid grid-cols-1 items-center gap-8 pb-10 pt-8 sm:pb-14 sm:pt-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:gap-10 lg:pt-20`} aria-labelledby="hero-title">
          <div>
            <h1 id="hero-title" className="text-[32px] font-semibold leading-[1.08] tracking-tight sm:text-[46px] lg:text-[54px]">{t.hero.title}</h1>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              ...
              <a href="#funciones" className={CTA_SECONDARY}>{t.hero.secondary}</a>
            </div>
          </div>
          <PhoneFrame label={t.hero.previewCaption}><TodayPreview t={t.preview.today} /></PhoneFrame>
        </section>
              ...
              {t.day.items.map((item, i) => {
                ...
                return (
                  <li key={item.title} className="flex gap-3.5 rounded-[18px] border border-[var(--border-dark)] bg-[var(--app-surface-dark-elevated)] p-4 md:flex-col md:p-5">
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--accent-muted)] text-[var(--accent-on-dark)]" aria-hidden="true"><Icon size={18} /></span>
        ...
        <FeatureSplit id="agenda" eyebrow={t.agenda.eyebrow} title={t.agenda.title} description={t.agenda.description} points={t.agenda.points} icon={CalendarDays} visual={<AgendaPreview t={t.preview.agenda} />} />
        <FeatureSplit id="mensajes" eyebrow={t.messages.eyebrow} title={t.messages.title} description={t.messages.description} points={t.messages.points} icon={MessageSquare} reverse tinted visual={<InboxPreview t={t.preview.inbox} />} caption={t.messages.previewLabel} />
        <FeatureSplit id="presencia" eyebrow={t.presence.eyebrow} title={t.presence.title} description={t.presence.description} points={t.presence.points} icon={Store} visual={<PresencePreview t={t.preview.presence} />} />
        ...
        <section id="para-quien" className="scroll-mt-20 border-t border-[var(--border-dark)] bg-[var(--app-surface-dark)]" aria-labelledby="forwho-title">
          <div className={`${SECTION} ${SECTION_PAD}`}>
            ...
            <ul className="mt-6 flex flex-wrap gap-2 md:mt-8 md:gap-2.5">
              {t.forWho.audiences.map((a) => <li key={a} className="rounded-full border border-[var(--border-dark)] bg-[var(--app-surface-dark-elevated)] px-3.5 py-2 text-[14px] font-medium md:px-4 md:py-2.5 md:text-[14.5px]">{a}</li>)}
            </ul>
          </div>
        </section>
        ...
        <section className={`${SECTION} ${SECTION_PAD}`} aria-labelledby="why-title">
          <div className="rounded-[24px] bg-[var(--accent-rich)] px-5 py-8 text-[var(--primary-foreground)] sm:rounded-[28px] sm:px-10 sm:py-14">
            ...
            <ul className="mt-6 flex flex-wrap gap-2">{t.why.pillars.map((p) => <li key={p} className="rounded-full border border-[color-mix(in_srgb,var(--primary-foreground)_35%,transparent)] px-3.5 py-1.5 text-[13.5px] font-semibold">{p}</li>)}</ul>
          </div>
        </section>
        ...
        <section className={`${SECTION} pb-14 text-center sm:pb-20`} aria-labelledby="final-title">
          ...
          <div className="mt-6 flex justify-center"><Link href={FINESSE_LOGIN_HREF} className={CTA_PRIMARY} data-cta="final">{t.finalCta.cta}<ArrowRight size={16} aria-hidden="true" /></Link></div>
        </section>
      </main>
      ...
      <footer className="border-t border-[var(--border-dark)] bg-[var(--app-surface-dark)]" data-finesse-footer>
        <div className={`${SECTION} flex flex-col gap-4 py-8 sm:flex-row sm:items-center sm:justify-between`}>
          <div><p className="text-[16px] font-semibold tracking-tight">{t.brand}</p><p className="text-[12.5px] text-[var(--text-secondary-light)]">{t.footer.tagline}</p></div>
          <div className="flex items-center gap-5 text-[13px]">
            <Link href={FINESSE_LOGIN_HREF} className="rounded-md font-medium text-[var(--text-primary-light)] hover:text-[var(--accent-on-dark)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent-primary)]">{t.footer.login}</Link>
...
function FeatureSplit({ id, eyebrow, title, description, points, icon: Icon, visual, caption, reverse = false, tinted = false }: { id: string; eyebrow: string; title: string; description: string; points: string[]; icon: typeof CalendarDays; visual: React.ReactNode; caption?: string; reverse?: boolean; tinted?: boolean }) {
  return (
    <section id={id} className={`scroll-mt-20 border-t border-[var(--border-dark)] ${tinted ? "bg-[var(--app-surface-dark)]" : ""}`} aria-labelledby={`${id}-title`}>
      <div className={`${SECTION} grid grid-cols-1 items-center gap-7 ${SECTION_PAD} lg:grid-cols-2 lg:gap-10`}>
        <div className={reverse ? "lg:order-2" : ""}>
          <p className={EYEBROW}>{eyebrow}</p><h2 id={`${id}-title`} className={H2}>{title}</h2><p className={LEAD}>{description}</p>
          <ul className="mt-5 space-y-2">{points.map((p) => <li key={p} className="flex items-center gap-2.5 text-[14.5px] sm:text-[15px]"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[var(--accent-muted)] text-[var(--accent-on-dark)]" aria-hidden="true"><Icon size={13} /></span>{p}</li>)}</ul>
        </div>
        <div className={reverse ? "lg:order-1" : ""}><div className="mx-auto w-full max-w-md">{visual}{caption ? <p className="mt-2.5 text-center text-[11.5px] text-[var(--text-tertiary-light)]">{caption}</p> : null}</div></div>
      </div>
    </section>
  )
}
