...
export function TareaForm({ open, onClose, onSuccess, data }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!titulo.trim()) return toast.error("Task title is required")
    setSaving(true)
    try {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Task title *" className="sm:col-span-2">
              <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Task title" className="input-field" autoFocus />
            </Field>
            <Field label="Project">
          ...
          <Field label="Description">
            <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} placeholder="Task description..." rows={3} className="input-field resize-none" />
          </Field>
