/**
 * Shared voice transcript store — pure, id-keyed. Extracted from
 * `app/voice-lab/transcript.ts` (CORE-VOICE-0B.1.1) so any voice surface
 * accumulates Realtime transcripts identically. Items are tracked by id so we
 * never blindly append on each update:
