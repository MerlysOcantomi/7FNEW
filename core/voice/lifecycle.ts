/**
 * Shared voice session lifecycle — pure, deterministic. Extracted from
 * `app/voice-lab/session-lifecycle.ts` (CORE-VOICE-0B.1.2a).
 *
 ...
 * so every voice client and its tests share one source of truth. The
 * `VoiceState` contract is untouched.
 */
...
export type LifecycleEvent =
  ...
  | { type: "disconnected" } // user pressed stop/disconnect
