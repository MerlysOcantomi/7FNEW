...
export function applicationLightContract(key: AppLightThemeKey): DesignContract {
  if (!isAppLightThemeKey(key)) throw new Error("Unsupported application light theme")
  return {
    ...DEFAULT_DESIGN_CONTRACT,
    brand: { name: key === "sevenef-pearl-blue" ? "sevenef" : "Finesse" },
    palette: { family: key, mode: "light" },
  }
}

export function resolveApplicationLightTokens(key: AppLightThemeKey): Record<string, string> {
  if (!isAppLightThemeKey(key)) throw new Error("Unsupported application light theme")
  const f = resolveDesignTokens(applicationLightContract(key))
  const d = APP_LIGHT_DETAILS[key]
  const aliases: Record<string, string> = {
    ...
    "--app-surface-hover": "rgba(12, 26, 43, 0.055)",
    "--app-surface-active": "rgba(12, 26, 43, 0.09)",
    "--app-control-radius": "10px",
    "--app-shadow-subtle": f["--fd-shadow"],
    "--premium-glow": `color-mix(in srgb, ${d.glow} 14%, transparent)`,
    "--premium-edge": d.glow,
    "--premium-metal": d.metal,
    "--premium-glass": f["--fd-glass"],
    "--premium-panel-fill": `linear-gradient(150deg, ${f["--fd-strong"]} 0%, ${f["--fd-surface"]} 82%)`,
    "--premium-highlight": "inset 0 1px 0 rgba(255, 255, 255, 0.82)",
    ...
    "--inbox-chat-bubble-outbound-shadow": "rgba(12, 26, 43, 0.08)",
    "--inbox-chat-bubble-outbound-shadow-hover": "rgba(12, 26, 43, 0.12)",
    "--inbox-chat-meta-outbound-bg": f["--fd-strong"],
    ...
    "--inbox-composer-input": f["--fd-surface"],
    ...
    "--inbox-divider": "rgba(12, 26, 43, 0.10)",
    "--inbox-list-divider": "rgba(12, 26, 43, 0.10)",
...
export function applicationLightStyles(): string {
  return APP_LIGHT_THEME_KEYS.map((key) => {
    const declarations = Object.entries(resolveApplicationLightTokens(key))
