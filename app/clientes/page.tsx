    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-30 bg-card border border-border rounded-lg shadow-lg overflow-hidden min-w-[170px]">
            <Link
              href={`/clientes/${clientId}`}
              onClick={() => setOpen(false)}
              className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground hover:bg-background transition-colors"
            >
              <ArrowUpRight size={13} strokeWidth={1.75} className="text-muted-foreground" />
              View client
            </Link>
            <Link
              href="/proyectos"
              ...
              className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground hover:bg-background transition-colors"
            >
              <FolderKanban size={13} strokeWidth={1.75} className="text-muted-foreground" />
              New project
            </Link>
          </div>
        </>
        ...
        <div className="px-5 md:px-8 py-7 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border bg-card text-sm text-foreground hover:border-primary transition-colors min-w-[130px] justify-between"
              >
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">All clients</h2>
              <span className="text-xs text-muted-foreground">{clientes.length} client{clientes.length !== 1 ? "s" : ""}</span>
            </div>
              ...
              <>
                ...
                <div className="hidden sm:block bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                  <div className="grid grid-cols-12 px-5 py-2.5 border-b border-muted bg-background">
                    ...
                    <span className="col-span-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Company</span>
                    <span className="col-span-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Contact</span>
                    <span className="col-span-1 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Status</span>
                    <span className="col-span-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Projects</span>
                    <span className="col-span-1 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Updated</span>
                  ...
                  {clientes.map((c: any, i: number) => {
                    ...
                    return (
                      <div
                        key={c.id}
                        className={cn(
                          "grid grid-cols-12 items-center px-5 py-4 hover:bg-background transition-colors",
                          i < clientes.length - 1 && "border-b border-muted"
                        )}
                      >
                        <div className="col-span-3 flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-lg bg-accent flex items-center justify-center shrink-0">
                            <span className="text-[11px] font-bold text-primary">{getInitials(c.nombre)}</span>
                          </div>
                          <div className="min-w-0">
                            <Link href={`/clientes/${c.id}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors truncate block">
                              {c.nombre}
                            </Link>
                            {c.empresa && (
                              <p className="text-[10px] text-muted-foreground truncate">{c.empresa}</p>
                            )}
                          </div>
                        </div>
                        <span className="col-span-2 text-sm text-muted-foreground truncate">{c.empresa || "—"}</span>
                        <div className="col-span-2 text-sm text-muted-foreground truncate">
                          {c.email ? (
                            <a href={`mailto:${c.email}`} className="hover:text-primary truncate block">{c.email}</a>
                          ) : (
                            c.telefono || "—"
                          )}
                        </div>
                        ...
                        <div className="col-span-2 flex items-center gap-1.5">
                          <FolderKanban size={13} className="text-muted-foreground" strokeWidth={1.75} />
                          <span className="text-sm text-foreground">{count.proyectos ?? 0} {(count.proyectos ?? 0) !== 1 ? "projects" : "project"}</span>
                        </div>
                        <span className="col-span-1 text-xs text-muted-foreground">{formatDate(c.updatedAt)}</span>
                  ...
                  {clientes.map((c: any) => {
                    ...
                    return (
                      <div key={c.id} className="bg-card rounded-xl border border-border shadow-sm p-4">
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-9 h-9 rounded-lg bg-accent flex items-center justify-center shrink-0">
                              <span className="text-xs font-bold text-primary">{getInitials(c.nombre)}</span>
                            </div>
                            <div className="min-w-0">
                        ...
                        <div className="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-muted">
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="text-xs text-muted-foreground">{count.proyectos ?? 0} {(count.proyectos ?? 0) !== 1 ? "projects" : "project"}</span>
                            <span className="text-[10px] text-muted-foreground">Updated {formatDate(c.updatedAt)}</span>
                          </div>
                          <Link
                            href={`/clientes/${c.id}`}
                            className="flex items-center gap-1 text-xs text-primary font-medium hover:text-primary/80 transition-colors shrink-0"
                          >
                            View <ArrowUpRight size={11} />
                          </Link>
                        </div>
