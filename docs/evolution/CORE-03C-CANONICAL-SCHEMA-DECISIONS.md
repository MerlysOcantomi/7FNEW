...
- **Status:** decision record, updated by **CORE-03C-2A** (2026-08-16, Stage 1: C1–C3) and **CORE-03C-2B** (2026-08-18, Stage 2: C4–C7) — see the *Implementation status* blocks in §13. Stage 3 (M1/M2) onward remains unimplemented. **No migration has been applied to Turso, no `_prisma_migrations` exists on Turso, no remote database was touched, and no data changed.**
...
> - ~~C4 and everything after remain pending~~ **Superseded — see the CORE-03C-2B block below.**

> **Implementation status — CORE-03C-2B (2026-08-18): Stage 2 complete (C4–C7).** All verification ran exclusively against throwaway local SQLite databases built from the migration history; **no migration was applied to Turso, Neon or any shared environment; no production data or production `_prisma_migrations` ledger was touched; `prisma/schema.prisma`, `prisma.config.ts`, `migration_lock.toml` and `0_baseline/migration.sql` are unchanged in this mission.**
> - **C4 done** — `prisma/migrations/1_add_missing_indexes/migration.sql`: exactly the 21 immediately applicable missing indexes, extracted verbatim from the canonical DDL and cross-checked name-by-name against CORE-03B (no `IF NOT EXISTS`; rollback documented as comments). From-scratch checkpoint: 48 tables / 82 indexes, integrity ok, 0 FK violations. Commit `aa87c847d59dfa5dee79bee711b30cbf89399a0f`.
> - **C5 done** — `prisma/migrations/2_add_link_columns/migration.sql`: nullable `InboxTodo.workspaceTaskId` + `InboxTodo_workspaceId_workspaceTaskId_idx`, nullable `QRCode.workspaceId` + `QRCode_workspaceId_module_recordId_idx`; no defaults, no FKs, no rebuilds, no DML. Checkpoint: 48 tables / 84 indexes, both columns nullable/default-free/FK-free. **QR runtime audit: the three `/api/qr/*` routes were already correctly workspace-scoped** (POST stores the `requireWriteAccess`-resolved workspace and ignores any client-supplied value; GET/DELETE filter by `workspaceId`) — routes unchanged; regression coverage added in `app/api/qr/qr-security.test.ts` (5 tests incl. hostile body+header override attempts and cross-workspace GET/DELETE, against a history-built DB). Commit `10bf5eccc6286858e8643b9b9e841c6fb2439432`.
> - **C6 done** — `prisma/migrations/3_create_portal_tables/migration.sql`: exactly 4 CREATE TABLE (ClientAsset → ClientRequest → ClientRequestAsset → ForteSnapshot, dependency-safe order) + 9 CREATE [UNIQUE] INDEX, extracted verbatim from the canonical DDL (never from a drift script); declared FKs preserved (Cliente CASCADE ×2, Proyecto SET NULL, ClientRequest CASCADE, Workspace CASCADE) and both ForteSnapshot indexes kept (`_key` unique + `_idx`). Checkpoint: **52 tables / 93 explicit indexes**, integrity ok, 0 FK violations, all four tables structurally identical to the canonical schema and empty; no pre-existing table rebuilt. Runtime patterns proven against the history-built DB in `app/api/cliente/requests/portal-tables.integration.test.ts` (nested asset create/include, scoped asset list, real snapshot-store round-trip, FK cascade enforcement); `npm run test:forte-business` green. Commit `3732f19a51f3d8b03ed3813568df7e7329721975`.
> - **C7 done** — `scripts/build-db-from-history.ts` (+ `scripts/build-db-from-history.test.ts`), `prisma/migrations/drift-manifest.json`, `npm run db:verify-history`, and a credential-free CI step ("Verify migration history"). The verifier deploys the full history into a fresh temp SQLite DB, checks the ledger records exactly the four expected migrations, checks 52/93/integrity/FK-clean, builds a second DB from the canonical schema DDL, computes the semantic drift (columns by name — type/nullability/default/PK; FKs with actions; indexes with uniqueness and column lists; physical column order deliberately excluded), and fails closed on any unmanifested drift, stale/malformed/duplicated manifest entry, satisfied-expiry-with-live-drift, or missing/unexpected migration. **Drift manifest: 57 entries, all mapping 1:1 to CORE-03B/CORE-03C-documented differences** — 41 foreign-key (37 never-deployed FKs + 2 `ConversationRead` FK pairs deployed with `ON UPDATE NO ACTION` instead of `CASCADE`, 4 entries) with expiry `NEON_CUTOVER`; 7 column-nullability (the §7 matrix exactly) with expiry `AUDIT_DECISION:M2`; 9 column-default (CORE-03B's 6 informational defaults + the hand-written-DDL `updatedAt` defaults on ClientAuth/Vertical/Workspace) with expiry `NEON_CUTOVER`. Negative tests prove non-zero exits for unknown drift, stale entry, malformed + duplicate entries, missing migration and unexpected migration. Commit `34790212444cc9adbc2513c62514510440686f20`.
> - **Next stage: M1** — the read-only, aggregate-only data audit (§12). D2–D7 remain open where applicable. **No claim is made that Prisma Migrate adoption on Turso has been proven** — that remains the §10 UNVERIFIED PROVIDER CAPABILITY pending its rehearsal.

> **Implementation status — CORE-03C-M1 (2026-08-18/19): Stage 3, M1 complete.** The read-only aggregate audit ran against production Turso on 2026-08-19 (strictly SELECT/PRAGMA; zero writes; no `_prisma_migrations` created; post-run check confirmed the deployed 48-table/61-index baseline unchanged). Full evidence: `CORE-03C-M1-DATA-LEGACY-AUDIT.md` §21. Headlines: D2 trio = 0 rows; the four §7 tightening columns = 0 violating rows; both 2B link columns confirmed absent in production (InboxTodo create HIGH revalidated); FK orphans concentrated in 3 relations; 0 cross-tenant mismatches.

> **Implementation status — CORE-03C-D6-REHEARSAL (2026-08-20): rehearsal PASS 8/8 — production migration still NOT authorized.** The adoption rehearsal ran end-to-end against a disposable local libSQL server (`sqld 0.24.32`, loopback-only, fail-closed guard): legacy baseline (48/61/no ledger) → ledger adoption → migrations 1–3 (validated individually) → 52/93 → idempotent re-run → real-service InboxTodo dual-write E2E (the production HIGH resolves after `2_add_link_columns`) → transaction-rollback proof → snapshot restore rehearsed and verified. **Key mechanism finding: Prisma 7.4.1 `migrate` accepts only `file:` URLs for sqlite (P1013 on `http://`/`libsql://`), so the §10 UNVERIFIED PROVIDER CAPABILITY resolves as NOT SUPPORTED as a direct CLI flow against Turso** — the rehearsed mechanism is a ledger-compatible applier verified row-equal against a real Prisma CLI reference run (tool: `scripts/rehearse-core-03c-d6.ts`, `npm run db:rehearse-d6`). Record: `CORE-03C-D6-MIGRATION-REHEARSAL.md`. Zero connections to production; zero writes anywhere but the disposable server; D6's production timing remains an open owner decision.

> **Implementation status — CORE-03C-M2 (2026-08-20): Stage 3, M2 complete — decisions closed, nothing executed.** With the M1 evidence the owner closed: **D2 approved** (retire the trio, gated on a fresh `COUNT(*) == 0` pre-flight), **D5 approved with mandatory pre-migration re-check** (no invented values), **D3 policy approved** (DETERMINISTIC/AMBIGUOUS/ORPHANED/LEGITIMATE_NULL classification, deterministic-only auto-repair, never a default workspace), InboxTodo NO_MATCH = no synthetic WorkspaceTask, QRCode = deterministic-only backfill, ChannelConnection orphans = SetNull convergence, `User.workspaceId` = LEGITIMATE_NULL (WorkspaceMember is canonical). D6 remains gated on the rehearsal, now fully specified. Record, per-model repair plans and reproducible gates: `CORE-03C-M2-FINAL-DATA-LEGACY-DECISIONS.md`. **Zero writes to Turso, zero migrations applied, zero data changed in M2.**
...
- **D2 — Retirement of ClientProject / ClientInvoice / ClientFile. ✅ RESOLVED (CORE-03C-M2, 2026-08-20): the owner approved retirement.** M1 measured 0 rows in all three tables (plus the standing 0-code-reference evidence), so retirement needs no data migration. Gated execution: a fresh `COUNT(*) == 0` pre-flight per table immediately before the retiring mission; any row → STOP. Nothing dropped yet. *(Original recommendation kept for history: keep models and tables until the §12/M1 row counts are known; retire only if counts are 0 or the data is confirmed obsolete, with owner approval — both conditions are now met at decision level.)*
- **D3 — FK completion on Turso.** Full 37-FK completion on SQLite = heavy rebuilds. *Recommendation:* complete FKs only in the Neon history; optionally an audited Inbox-table subset on Turso after §12-B. **Update (CORE-03C-M2): the repair-policy half is ✅ RESOLVED** — deterministic-only classification (DETERMINISTIC/AMBIGUOUS/ORPHANED/LEGITIMATE_NULL), never a default workspace; M1 narrowed the dirty set to 3 relations (`Activity.workspaceId`, `Conversation.connectionId`, `Message.connectionId`). The *placement* half (Neon-first vs. audited Turso subset) stays as recommended here.
- **D4 — `User.googleId` retirement.** Unused by runtime; declared for safety. *Recommendation:* confirm no external consumer, then schedule a drop mission; zero urgency. **Update (CORE-03C-M2): `User.workspaceId` joins the same gated-retirement family** — confirmed LEGITIMATE_NULL, unread by runtime, canonical membership lives in `WorkspaceMember`; do not backfill.
- **D5 — `Factura.items` / `Documento.url` NULL repair values. ✅ RESOLVED (CORE-03C-M2, 2026-08-20): tightenings approved with mandatory pre-migration re-check.** M1 measured 0 violating rows in all four §7 tightening columns, so no repair backfill is needed at current data; the executing mission re-runs the D5 gate immediately before migrating and STOPs on any new incompatible row — no invented values, no semantically false defaults.
...
**Status after CORE-03C-2B:** D1 is resolved (ForteSnapshot = CANONICAL_ADD, owner-approved); Stages 1 and 2 (C1–C7) are implemented and locally verified — see §13's implementation-status blocks. The from-scratch history now reaches the full 52-table / 93-index structure, the four CANONICAL_ADD tables exist in the history with their declared FKs and indexes, and CI enforces the drift manifest on every push. Open owner decisions: **D2–D7**. Next: **M1** — the read-only, aggregate-only data audit (§12) — then Stage 3 onward strictly in order. Standing guarantees, reaffirmed for CORE-03C-2A as well: **zero writes to Turso, zero creation of `_prisma_migrations` anywhere but throwaway local databases, zero migrations applied to any shared environment, zero changes to runtime code or configuration** (the only code-adjacent change is the declarative schema convergence of already-deployed objects).
